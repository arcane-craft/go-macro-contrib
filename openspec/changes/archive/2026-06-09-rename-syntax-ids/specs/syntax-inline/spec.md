...
`inline` 包 MUST 提供 `//macro: inline` 标注的 `InlineExpand`。展开器 MUST 根据 `ctx.StubName()` 确定期望返回值个数 `n`（`Inline0`→0，`Inline`→1，`Inline2`→2，`Inline3`→3），并 MUST 用 `types.Info` 计算内层被调函数返回值个数；二者不一致时 MUST 返回错误并提示应使用的桩名。
